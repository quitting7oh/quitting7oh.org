...
const SIMPLE_STORAGE_KEY = 'sr17-simple-cross-taper-v2';
const SIMPLE_STORAGE_VERSION = 2;
...
Starting point:
- 7-OH: ${formatDose(inputs.ohPerDose)} mg × ${inputs.ohDosesPerDay}/day
- SR-17: ${formatDose(srPerDose)} mg × ${inputs.ohDosesPerDay}/day
...
function SimpleSr17Calculator() {
          ...
          setInputs({
            ...
            ohDosesPerDay:
              typeof stored.ohDosesPerDay === 'number'
                ? Math.min(24, Math.max(1, Math.round(stored.ohDosesPerDay)))
                : SIMPLE_DEFAULTS.ohDosesPerDay,
            highDoseSr17:
              typeof stored.highDoseSr17 === 'number'
                ? Math.min(100, Math.max(75, stored.highDoseSr17))
                : SIMPLE_DEFAULTS.highDoseSr17,
  ...
  const srPerDose = recommendedSr17PerDose(
    inputs.ohPerDose,
    inputs.highDoseSr17,
  );
  const schedule = useMemo(
    ...
    [inputs.ohDosesPerDay, inputs.ohPerDose, inputs.taperDays, srPerDose],
  );
  ...
  const handleCopy = async () => {
    if (srPerDose === null) return;
    const text = buildSimpleScheduleText(inputs, srPerDose);
    try {
    ...
    <div className="space-y-5">
      <div className="overflow-hidden rounded-xl border border-border bg-card print:hidden">
        <div className="border-b border-border/75 px-5 py-5 sm:px-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Enter one dose. The SR-17 frequency starts on the same schedule.
          </p>
        </div>
            ...
            {srPerDose === null ? (
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <output
                    id="simple-sr-dose"
                    htmlFor="simple-oh-dose"
                    className="mt-1 block font-display text-4xl font-semibold text-foreground"
                  >
                    {formatDose(srPerDose)} mg
                  </output>
                  <p className="mt-1 text-sm text-muted-foreground">
                    {inputs.ohDosesPerDay}× daily, matching your current frequency
                  </p>
                </div>
                <p className="text-sm font-semibold text-primary">
                  {tabletLabel(srPerDose / 50)} from a 50 mg tablet per dose
                </p>
              </div>
            )}

            {inputs.ohPerDose >= 1000 && (
              <div className="mt-4 max-w-xs">
                <Label htmlFor="simple-high-sr-dose">
                  Adjust SR-17 dose between 75 and 100 mg
                </Label>
                <NumberInput
                  ...
                  max={100}
                  step={25}
                  value={inputs.highDoseSr17}
                  onValueChange={(value) => set('highDoseSr17', value)}
                  className="mt-1.5"
                />
              </div>
            )}
          ...
          <fieldset>
            ...
            <div className="mt-2 grid grid-cols-3 gap-2">
              {SIMPLE_SR17_TAPER_DAYS.map((days) => (
                <label
                  key={days}
                  className={`flex min-h-12 cursor-pointer items-center justify-center gap-2 rounded-lg border px-3 text-sm font-bold transition-colors ${
      ...
      {schedule && srPerDose !== null && (
        <div className="overflow-hidden rounded-xl border border-border bg-card">
          ...
          <div className="flex flex-wrap items-center gap-3 border-t border-border/75 px-5 py-4 sm:px-6 print:hidden">
            ...
            <p className="text-xs text-muted-foreground">
              Total across the taper: {formatDose(schedule.totalSrMg)} mg SR-17
            </p>
          </div>
        </div>
      )}
    ...
    <div className="not-prose my-6 space-y-6">
      <div className="print:hidden">
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
          {mode === 'simple'
            ? 'Simple calculates a starting SR-17 dose and builds a complete 7, 10, or 14-day cross-taper.'
